...
func TestInitContainer(t *testing.T) {
	...
	config.pbsproSchemaInitContainerImage = pbsproSchemaInitContainerImageDefault
	config.pbsproSchemaInitContainerTemplateFile = pbsproSchemaInitContainerTemplateFileDefault
			...
			kubecluster: &v1alpha1.KubeCluster{
				Spec: v1alpha1.ClusterSpec{
					ClusterReplicaSpec: map[v1alpha1.ReplicaType]*v1alpha1.ReplicaSpec{
						SchemaReplicaTypeServer: {
							Replicas: pointer.Int32(1),
						},
					},
				},
			},
			rtype:       SchemaReplicaTypeServer,
			index:       "0",
			expected:    1,
			exepctedErr: nil,
		},
...
func TestGetInitContainer(t *testing.T) {
	...
	for _, t := range testCases {
		controllerAddress := common.GenGeneralName(t.kubecluster.Name, SchemaReplicaTypeServer, strconv.Itoa(0))
