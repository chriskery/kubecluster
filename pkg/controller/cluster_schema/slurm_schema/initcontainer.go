...
func setInitContainer(
	...
	cpSlurmCmdComand := fmt.Sprintf("if [ -d %s ] && [ -d %s ];then cp -r %s/* %s;fi", SlurmCmdDir, EmptyVolumeMountPathInInitContainer, SlurmCmdDir, EmptyVolumeMountPathInInitContainer)
	if rtype == SchemaReplicaTypeController {
		if index == "0" {
			cpSlurmCmdComand = fmt.Sprintf("%s && %s", genMungeKeyCmd(), cpSlurmCmdComand)
		}
		initContainers[0].Command = []string{"sh", "-c", cpSlurmCmdComand}
	} else {
		initContainers[0].Command = []string{"sh", "-c", fmt.Sprintf("%s && %s", cpSlurmCmdComand, strings.Join(initContainers[0].Command, " "))}
	}
